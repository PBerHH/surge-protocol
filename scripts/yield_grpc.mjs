...
(async () => {
  ...
  const f = v.object.json.value ?? v.object.json;
  ...
  const commandOutputs = sim?.Transaction?.commandOutputs ?? sim?.transaction?.commandOutputs ?? sim?.commandOutputs;
  if (!commandOutputs) {
    console.log('DEBUG raw simulateTransaction result:');
    console.log(JSON.stringify(sim, (k, val) => typeof val === 'bigint' ? val.toString() : val, 2));
    process.exit(1);
  }
  const rv = commandOutputs[0]?.returnValues?.[0];
  const bytes = rv?.value?.value ?? rv?.bcs?.value ?? rv;
  let rate = 0n;
  for (let i = bytes.length - 1; i >= 0; i--) rate = (rate << 8n) | BigInt(bytes[i]);

  const value = (ha * rate) / 1_000_000n;
